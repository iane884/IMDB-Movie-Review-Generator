# -*- coding: utf-8 -*-
"""CS150-a8-yourname.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/129RGAB7LurhpXr6Iemrdtzs_DkfUwlmg
...
# Load the IMDB reviews (like in the Movies assignment!)
# "IN_COLAB" is like __main__, a section where you can do what you want and it won't affect grading
if IN_COLAB:
...
# Decoding reviews
# https://colab.research.google.com/github/cbernet/maldives/blob/master/imdb/imdb_embed.ipynb#scrollTo=4hv1vHrnMqHp
# Unlike the previous reviews, these aren't stored as text, they are stored as...
# lists of numbers!
# The reviews are *encoded* as numbers because that makes them easier to train networks on
# But we might still want to see what text they contain, so we can *decode* them
...
def add_to_dictionary(dictionary, word):
  ...
  # similar to in the Movies assignment
...
def generate_random_text(freq_dict, count=10):
  ...
  # * Use random.choice (like from the Tracery assignment)
  #    (https://www.programiz.com/python-programming/modules/random)
  #   to add random words to a list until you have "count" number of words
  # * return that list, joined with " " so that you return a single string
  # Note: You may see some "special" tokens, like <START> and <UNK> (unknown)
  #   which were added by the tokenizer
...
def generate_random_weighted_text(freq_dict, count=10):
  ...
  # whether the word was "the" or "cheesy"
  # We can get more natural-looking results with a *weighted* distribution
  # Fortunately, new (3.6+) versions of Pyhton let us make *weighted choices*
...
# Here is a good 12 minute explanation of using Markov Chains for text generation
...
def create_markov_dictionary(samples):
  ...
  #         (Remember that we have an easy way to see if an element is in a list
...
# Lets generate some text with Markov chains!
...
# (I'm skipping training a neural network for this quarter,
# but you can read about it here https://towardsdatascience.com/a-deep-learning-approach-in-predicting-the-next-word-s-7b0ee9341bfe)
# In this section, we will use colab to download and use a pretrained model hosted on Huggingface, GPT2
# You can learn about GPT2 here https://www.youtube.com/watch?v=XynJ-gM6aD0&t=297s
# We will be using it interactively here, and running some small experiments
# ... but first, we have to use colab to install the "transformers" library, and import some parts of it

# WARNING: COMMENT THIS OUT BEFORE DOWNLOADING
...
if IN_COLAB:
  ...
  # When ones are *longer* than you might expect?
  for thing in things:
    text = f"I love {thing}"
...
if IN_COLAB:
  ...
  for thing in things:
    ...
    print(text, tokens)
    print(" -> ", decoded_text)
  # What does this tell us about how the text is encoded?
...
if IN_COLAB:
  ...
  def generate_gpt2_output(prompt, max_tokens_to_generate=5, count=1, temperature=0.7):
    ...
    # top_k and top_p are other tuning parameters (https://huggingface.co/blog/how-to-generate)
...
def run_experiment(prompt, options):
  ...
  # What is the next text it generates?

  # For each option
  #   create a modified version of the prompts with all the [X] replace with that option
  #   (how to replace text: https://www.geeksforgeeks.org/python-string-replace/)
  #
  ...
  # Try adjusting the temperature to see if you get better results (won't affect testing)
